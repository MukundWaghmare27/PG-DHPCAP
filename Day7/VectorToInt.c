...
int main(int argc, char** argv) {
    ...
    if (rank == 0) {
        ...
        MPI_Send(arr, 1, vector_type, 1, 0, MPI_COMM_WORLD);  //only 3 data it sending (count * blocklength)
        ...
        MPI_Recv(arr, 3, MPI_INT, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);   //so receiving only 3 data's //blocklength * count = 3 (total data receiving)
